...
def prepare_training_arrays(npz_path: str | Path, seed: int = 0):
    ...
    mu = data["mu_hat"].astype(np.float32)
    k = data["k_hat"].astype(np.float32)
...
if __name__ == "__main__":
    train_model(
        data_path=ROOT / "datasets" / "adaptive_estimator_dataset.npz",
        save_dir=ROOT / "models" / "fno_zeta_adaptive" / "adaptive_run_001",
        epochs=100,
        lr=4e-3,
        weight_decay=1e-6,
        n_modes=32,
